...
def split_row(row, mid, page_width, col_gap=8.0, band=(0.30, 0.70)):
    ...
    a wide gap. The gap is accepted anywhere in the middle band of the page, so
    the split does not depend on how precisely the boundary was estimated.
    """
    prev = None
    for c in row:
        x0 = float(c["x0"])
        if prev is not None and (x0 - prev) >= col_gap:
            centre = (x0 + prev) / 2
            if page_width * band[0] <= centre <= page_width * band[1]:
...
def word_gap_threshold(rows, override_ratio=None):
    ...
    spaces = sum(1 for c in chars if not c['text'].strip())
    if spaces >= max(10, 0.01 * len(chars)):
        # space glyphs already exist; kerning gaps inside DOIs or numbers
        # must not be turned into spaces
        return None
    if override_ratio:
    ...
    with pdfplumber.open(args.source) as pdf:
        for pno, page in enumerate(pdf.pages, start=1):
            ...
            if not single:
                ...
                split = []
                for row in rows:
                    split.extend(split_row(row, mid, float(page.width)))
                rows = split
