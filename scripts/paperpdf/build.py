...
class TrackedImage(Image):
    def __init__(self, filename, asset):
        super().__init__(str(filename), width=asset['width_pt'], height=asset['height_pt'])
        self.asset = asset
...
def build(paper, sample=False):
    ...
    def paragraph(text, style, bid, cn=None):
        ...
        markup, plain = font.format(text, cn=cn, bold=style in {'heading','title_en'}, size=sty[style].fontSize)
    ...
    for b in blocks:
        ...
        if typ in {'figure','table','equation'}:
            ...
            inner_width = max(W-2*margin, asset['width_pt'])
            with chinese_wrapping():
                needed_height = sum(p.wrap(inner_width, 100000)[1]+p.getSpaceBefore()+p.getSpaceAfter() for p in parts)
            if asset['width_pt'] > W-2*margin+.01 or needed_height > H-2*margin:
                if paper.layout['oversize'] != 'page':
                    raise ValueError(f"Asset needs a larger page (never scaled): {asset['id']}")
                name = f'asset_{bid}'
                w = max(W, asset['width_pt']+2*margin)
